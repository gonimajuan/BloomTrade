...
| D17 | **Coverage aceptado a ~60% global vs 80% objetivo** de CONVENTIONS §7.1 (servicios/dominio). Las clases más afectadas son los validadores del paquete `auth` (`StrongPassword`/`PasswordPolicyValidator`, `ConsistentDocumentNumber`/`DocumentNumberValidator`) y los DTOs/records. | Trade-off deliberado por plazo del MVP (decisión humana 2026-05-19 al recibir el reporte de SonarCloud). El gap se prioriza por **velocidad de entrega** sobre cobertura; las lógicas críticas (`RegisterService`, `WelcomeEmailDispatcher`, política de password) sí están unit-testeadas. Si quedan días al cierre del Sprint 1, sumar tests parametrizados sobre los validadores debería elevar coverage rápido. |
