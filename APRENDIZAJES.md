...
## Día 12 — Revamp UI completo glassmorphism + violet accent (2026-05-27)

### El revamp sin SDD formal: cuando el proceso debe adaptarse al artefacto

Al arrancar el revamp tuve la duda de si aplicar SDD completo (`specs/HU-XX-revamp-ui/SPEC.md` + `plan.md` + `tasks.md`) o un mini-doc en `docs/ui-revamp/PLAN.md` sin tasks granulares. Decidí el segundo. **Razón**: el SDD es un contrato sobre comportamiento — endpoints, DTOs, validaciones, flujos. Un revamp visual no tiene "contratos" en ese sentido — el artefacto SON los pixeles, no los endpoints. Forzar un SPEC con §5 "Flujos" y §6 "Contratos API" sería burocracia vacía porque los flujos no cambian y los contratos quedan intactos. **Lección sobre el alcance del SDD**: el proceso debe reflejar la naturaleza del artefacto. Para HUs funcionales (cambian comportamiento), SDD completo. Para artefactos visuales (cambian forma sin cambiar comportamiento), plan + lotes + validación humana es suficiente. **Heurística replicable**: si el artefacto no tiene tests automatizados que lo validen (los tests visuales son el ojo humano), el SDD pesado pierde su retorno. La pregunta que me hice: "¿qué decisión técnica del SPEC va a influenciar el código y no es obvia desde un screenshot?" — respuesta: ninguna. Eso me dio confianza para saltar specs/tasks sin sentirme heretico contra la regla del proyecto.

### Glassmorphism necesita atmósfera detrás — `GlassBackground` como decisión arquitectónica, no decorativa

Mi primer impulso al diseñar los primitives fue hacer cards con `bg-white/10 backdrop-blur`. Pero al armar el primer mockup mental, me di cuenta que sobre el fondo plano `bg-slate-950` actual eso se ve como "rectángulos translúcidos sin punch" — el `backdrop-filter` NO tiene nada que difuminar. **El glassmorphism no funciona sin algo detrás del vidrio**. Solución: `<GlassBackground>` wrapper en `App.tsx` con 3 orbes blurred (violet/cyan/fuchsia) + animaciones CSS keyframes de 24–32s. Ahora cada Card glass del producto hereda automáticamente la atmósfera. **Lección sobre estilo de sistema vs estilo de componente**: el glassmorphism no es styling de cards, es styling de sistema. Si una Card glass se ve plana, el problema NO es la Card — es la atmósfera. Hacer la decisión a nivel arquitectónico (un solo wrapper en root) me ahorró tener que poner "fondo decorativo" en cada page. **Heurística replicable**: cuando un efecto visual depende de qué hay detrás (blur, transparency, mix-blend-mode), la solución debe vivir en el contenedor que envuelve todo, no en los componentes hoja. La performance también ayudó: 3 divs `fixed` blurred + CSS keyframes = GPU composited, cero JS loop, cero re-render React.

### Lotes como unidad de validación escaló a 5 lotes en una sesión

Implementé 5 lotes (A→E) + Lote F de cierre en una sola sesión, ~5 horas de trabajo activo. Cada lote: lectura paralela de archivos → batch write paralelo → build → tests → reporte al usuario con summary de cambios y métricas (modules, bundle size, test count). **Beneficio principal**: cada validación humana al cierre de lote es un punto de back-out posible. Si Lote D hubiera roto algo grande (regressions en tests, build error, conflict de tipos), el rollback es a Lote C verde. **Sin esa cadencia**, la sesión habría sido un commit gigante de 40 archivos sin checkpoint intermedio. **Lección sobre escalabilidad de la cadencia SDD**: la regla `feedback-cadencia-sdd` ("implementar en lotes, validar en hitos") escala bien a sesiones largas con muchos archivos. La alternativa (validar archivo por archivo, "next.js way") habría hecho la sesión 10× más lenta. La alternativa contraria (validar al final solo) habría sido más rápida pero sin red de seguridad. **Heurística replicable**: el tamaño correcto del lote es "lo más grande que puedo describir en un párrafo + tests verdes". Si el reporte requiere 5 párrafos, el lote es demasiado grande para validar coherentemente.

### Borrar código como decisión emergente: D3 del PLAN eliminó 180 LOC del Lote E

`OrderConfirmationToast.tsx` (180 LOC + lógica embebida de palette/icons/render condicional) fue **borrado** en Lote E y reemplazado por 3 líneas de `sonner.toast.success/info` inline en `TradePage`. El plan original decía "refactor", no borrar. **La señal que detecté**: tras Lote B introducir sonner global, el componente custom cubría un caso de uso ya resuelto por una decisión previa. Mantenerlo (aunque migrado a glass) habría dejado dos sistemas paralelos de feedback (sonner para cancel, custom toast para confirm). **Lección sobre cuándo borrar > refactorear**: durante un refactor, preguntar "¿esto sigue siendo necesario?" — no solo "¿cómo lo mejoro?". El refactor mantiene complejidad cognitiva (1 componente más que aprender) sin agregar valor (sonner ya da todo lo que necesito). **Heurística replicable**: cuando un refactor toca un componente cuya funcionalidad solapa con una decisión arquitectónica nueva (toast global, modal system, theme provider), evaluar borrar antes de refactorear. La señal específica: "este componente cubre el mismo caso de uso que X que ya hicimos". 180 LOC menos = menos contexto para futuros agentes + menos código para mantener.

### El patrón "sr-only radio + label-as-button" como convención emergente del Lote E

BUY/SELL toggle en `OrderForm`, canal de notificación en `ProfilePage` (3 opciones), tickers de interés en `ProfilePage` (25 checkboxes). Tres usos del mismo pattern en la misma sesión: `<input type="radio|checkbox" className="sr-only" .../>` + `<label className="glass + cursor-pointer + selected-state">...</label>`. **El conflicto que resuelve**: native radios/checkboxes son feos cross-browser (estilo OS), custom controls sin input nativo pierden semántica para screen readers + keyboard nav. **El pattern**: el input existe (sr-only oculta visualmente, NO `display:none` que rompería a11y) — el label es lo visualmente clickeable, captura clicks vía la asociación HTML `<label for>` natural. **Lección sobre patterns que emergen 3 veces en una sesión**: cuando un patrón aparece 3 veces, vale la pena formalizarlo (componente `<SegmentedControl>` o `<TogglePill>` reusable). NO lo hice en esta sesión porque cada uso tiene shape distinto (2 opciones, 3 opciones, 25 opciones grid). Para el revamp 1-day pragmático fue mejor inline; para una base de código a 2+ años, valdría la pena extraer. **Heurística**: 1 uso = inline, 2 usos = considerar extraer, 3+ usos = extraer. En este caso paré antes de extraer por presión de tiempo, lo registro como deuda emergente.

### Tipografía Space Grotesk: pequeño cambio, gran personality

Cambiar de system fonts (default Tailwind) a Space Grotesk Variable vía `@fontsource-variable/space-grotesk` agregó ~30kb gz al bundle (3 woff2 files: latin + latin-ext + vietnamese). **El cálculo de valor**: 30kb gz vs el efecto visual. **El resultado**: el producto SE VE distinto — geométrico, fintech-like, no como un dashboard genérico React/Material. La tipografía es el primer landmark visual que el cerebro registra al cargar la página, antes incluso que cards, colores, layout. **Lección sobre ROI tipográfico**: cambiar fonts es ROI alto a costo bajo. Si tuviera que recortar 30kb del bundle, recortaría de framer-motion (~50kb) antes que de tipografía. **Heurística replicable para diferenciación visual con presupuesto limitado**: priorizar **3 capas en orden** — tipografía + accent color + atmósfera de fondo. Estas 3 capas son el 80% del aesthetic percibido por el usuario en los primeros 2 segundos. Cards, sombras, microinteracciones son la siguiente capa de polish (importante pero no diferenciadora). El revamp BloomTrade gastó ese presupuesto en Space Grotesk + violet-500 + orbes blurred — combo que el usuario asocia con "premium fintech" instantáneamente vs el look "Tailwind defaults" original.

### `twMerge` + `cn()` como prerequisito de primitives con override

El `Card` primitive aplica `'rounded-2xl' + variantClasses[variant]` (donde `variantClasses.glass = 'border border-white/10 bg-slate-900/40 ...'`). Cuando un consumer escribe `<Card variant="glass" className="border-rose-500/30 bg-rose-500/10">` (error states semánticos), **`twMerge` resuelve los conflictos**: `border-white/10` se reemplaza por `border-rose-500/30`, `bg-slate-900/40` por `bg-rose-500/10`. Sin `tailwind-merge`, ambos border classes coexistirían en el HTML y el CSS final dependería del orden de las reglas Tailwind generadas — impredecible. **Lección sobre primitives con variants + override**: `twMerge` es no-opcional para esto. Sin él, el consumer debe conocer y replicar TODAS las clases del variant para anular UNA sola (escribir `border-rose-500/30 !important` o duplicar todo el variant). **Costo de la dep (~6kb gz) << beneficio**: variants composables sin que el consumer necesite leer la implementación del primitive. **Heurística para librerías de UI internas**: `clsx` + `tailwind-merge` (vía un util `cn()`) son tan fundamentales como Tailwind mismo. Sin ellos, los primitives son rígidos y obligan a duplicación. Es una decisión que toma 5 minutos al inicio y paga el resto del proyecto.

### Reflexión meta: 0 backend changes en 5 lotes — la prueba del módulo modular

El revamp tocó **solo `frontend/`** (más STACK.md como docs). **Cero cambios en `backend/`**, **cero en tests backend**, **cero en migraciones Flyway**, **cero en contratos API**. Esto valida pasivamente la arquitectura modular del proyecto: el frontend es un cliente del backend vía REST, ambos pueden evolucionar independientes. **Lección sobre validación pasiva de arquitectura**: si un revamp visual hubiera requerido cambios backend (por ejemplo, "necesito un campo `displayColor` en el DTO de Order para colorear la fila correctamente"), el módulo no era realmente independiente — la lógica de presentación estaría filtrándose al modelo de dominio. El test pasivo "0 backend changes en revamp UI completo" es un buen indicador de salud arquitectónica. **Reflexión meta sobre el aprendizaje acumulado**: 12 días de proyecto, 12 secciones en este APRENDIZAJES.md, ~80 reflexiones técnicas, 410 tests backend + 42 vitest, 0 deudas vivas críticas, MVP funcional + revamp visual cerrado. **Lo que cambió en mi forma de trabajar entre Día 0 y Día 12**: arranqué imaginando que SDD era ritual académico y terminé usándolo como herramienta de productividad. Arranqué temiendo agentes y terminé delegando lectura paralela de 11 archivos en una invocación. Arranqué desconfiando del LLM para tareas visuales y terminé corriendo un revamp completo glassmorphism en 5 horas. **El patrón meta**: las decisiones que parecían "lujos" del Día 0 (SDD, agentes paralelos, primitives reusables, design tokens) son las que aceleraron exponencialmente el Día 12. La inversión upfront paga compuesto en sesiones siguientes — siempre que la disciplina de mantener el sistema no se rompa. Lo que voy a llevar a futuros proyectos: invertir tiempo en infrastructure del proceso (specs, plan templates, design tokens, primitives) ES código de producción. No es overhead — es el código que escribe el otro código.
